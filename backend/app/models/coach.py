...
class Coach(Base):
    ...
    certifications = Column(
        JSON, nullable=True
    )  # List of coaching certifications
    specializations = Column(
        JSON, nullable=True
    )  # List of CoachSpecialization
    ...
    preferred_athlete_level = Column(
        JSON, nullable=True
    )  # beginner, intermediate, etc.
    coaching_style = Column(
        JSON, nullable=True
    )  # tags describing coaching approach
    ...
    athlete_connections = relationship(
        "CoachAthleteConnection", back_populates="coach"
    )
    ...
    def current_athlete_count(self) -> int:
        ...
        return len(
            [conn for conn in self.athlete_connections if conn.is_active]
        )
    ...
    def can_accept_athletes(self) -> bool:
        ...
        if (
            self.max_athletes
            and self.current_athlete_count >= self.max_athletes
        ):
            return False
        return True
...
class CoachAthleteConnection(Base):
    ...
    goals = Column(
        JSON, nullable=True
    )  # Shared goals for this coaching relationship
