...
class Recommendation(Base):
    ...
    metrics_to_track = Column(
        JSON, nullable=True
    )  # What to measure for progress

    # Personalization Factors
    based_on_factors = Column(
        JSON, nullable=True
    )  # What athlete data influenced this
    ...
    athlete_feedback = Column(
        JSON, nullable=True
    )  # Rating, comments from athlete

    # Effectiveness Tracking
    effectiveness_rating = Column(
        Float, nullable=True
    )  # Athlete's rating (1-5)
    measurable_improvement = Column(
        JSON, nullable=True
    ...
    )  # When recommendation becomes outdated
