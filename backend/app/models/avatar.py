...
class Avatar(Base):
    ...
    physical_features = Column(JSON, nullable=True)  # skin tone, body type, etc.
    ...
    sports_gear = Column(JSON, nullable=True)        # sport-specific equipment/clothing
    ...
    earned_items = Column(JSON, nullable=True)       # Items earned through achievements
    ...
    # Generated Assets
    avatar_image_url = Column(String(500), nullable=True)  # Generated avatar image
    thumbnail_url = Column(String(500), nullable=True)     # Small version
    last_generated = Column(DateTime, nullable=True)       # When image was last generated
...
class AvatarCustomization(Base):
    ...
    usage_count = Column(Integer, default=0)  # How many times this config was used
...
class AvatarItem(Base):
    ...
    description = Column(String(500), nullable=True)
    category = Column(String(50), nullable=False)  # clothing, accessory, hair, etc.
    subcategory = Column(String(50), nullable=True)  # shirt, pants, hat, etc.
    
    # Item Properties
    rarity = Column(String(20), default="common")  # common, rare, epic, legendary
    sport_specific = Column(String(50), nullable=True)  # If item is sport-specific
    ...
    requires_achievement = Column(String(100), nullable=True)  # Achievement needed to unlock
    
    # Item Data
    asset_data = Column(JSON, nullable=False)  # Visual/3D model data
    compatibility = Column(JSON, nullable=True)  # What styles/other items it works with
